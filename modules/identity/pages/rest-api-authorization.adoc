...
[NOTE]
====
Default configuration and rules are available in all editions but custom configuration and rules extensibility are only available in Subscription editions.
====

[WARNING]
====
From Bonita version 2022.1, Dynamic authorization checking is enabled by default in subscription editions.
From Bonita version 2024.1, Dynamic authorization checking is enabled by default in community edition.
====

For some API resources, the static authorization check may not be specific enough to properly define when the access is granted. In those situations, the static authorization check may be overriden using dynamic checks.
...
In community edition, a set of dynamic authorization checks is predefined for the resources of the bpm, identity, portal and living APIs.

=== In subscription editions

In subscription editions a dynamic authorization check for a resource is specified by a line in the file `dynamic-permissions-checks-custom.properties`.
...
To disable a single dynamic check for a method and resource, comment out the corresponding line in the file `dynamic-permissions-checks-custom.properties` (Subscription editions only).

To add a custom dynamic check for a method and resource, add your line in the file `dynamic-permissions-checks-custom.properties` following the example above (Subscription editions only).
