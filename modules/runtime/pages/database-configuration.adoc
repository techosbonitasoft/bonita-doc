...
Bonita supports PostgreSQL with all editions and MySQL, Microsoft SQL Server, and Oracle only with the Subscription edition.
...
* https://www.postgresql.org/docs/15/app-createdb.html[PostgreSQL]
...
* https://learn.microsoft.com/en-us/sql/relational-databases/security/authentication-access/create-a-database-schema?view=sql-server-ver16[SQL Server]
...
You can use Kerberos for database authentication.
Edit the file `setup/tomcat-templates/bonita.xml` by adding the following attributes to Resources:
